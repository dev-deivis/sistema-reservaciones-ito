...
const ReservacionCard = ({ reservacion, onCancelar, onModificar }) => {
  ...
  const abrirModal = () => {
    setFormMod({ fecha_inicio: isoToLocal(r.fecha_inicio), fecha_fin: isoToLocal(r.fecha_fin), motivo: r.motivo || '' });
    setErrorMod('');
    setMostrarModal(true);
  };

  const handleGuardar = async () => {
    if (!formMod.fecha_inicio || !formMod.fecha_fin) { setErrorMod('Debes ingresar ambas fechas.'); return; }
    if (new Date(formMod.fecha_inicio) >= new Date(formMod.fecha_fin)) {
      setErrorMod('La fecha de inicio debe ser anterior a la fecha de fin.'); return;
    }
    setGuardando(true); setErrorMod('');
    try {
      const { data } = await api.put(`/reservaciones/${r.id}`, {
        fecha_inicio: formMod.fecha_inicio, fecha_fin: formMod.fecha_fin, motivo: formMod.motivo,
      });
    ...
    <>
      ...
      {mostrarModal && (
        <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: '16px' }}>
          <div style={{ backgroundColor: 'white', borderRadius: '12px', width: '100%', maxWidth: '480px', boxShadow: '0 20px 25px -5px rgba(0,0,0,0.1)' }}>
            ...
            <div style={{ padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              <div>
                ...
                <input type="datetime-local" value={formMod.fecha_inicio} onChange={(e) => setFormMod({ ...formMod, fecha_inicio: e.target.value })} style={{ width: '100%', padding: '0.7rem', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '0.95rem', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.9rem', fontWeight: '500', color: '#374151' }}>Fecha y hora fin</label>
                <input type="datetime-local" value={formMod.fecha_fin} onChange={(e) => setFormMod({ ...formMod, fecha_fin: e.target.value })} style={{ width: '100%', padding: '0.7rem', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '0.95rem', boxSizing: 'border-box' }} />
              </div>
              <div>
                <label style={{ display: 'block', marginBottom: '0.4rem', fontSize: '0.9rem', fontWeight: '500', color: '#374151' }}>Motivo</label>
                <textarea rows={3} value={formMod.motivo} onChange={(e) => setFormMod({ ...formMod, motivo: e.target.value })} placeholder="Describe el motivo de la reservación" style={{ width: '100%', padding: '0.7rem', border: '1px solid #d1d5db', borderRadius: '8px', fontSize: '0.95rem', resize: 'vertical', boxSizing: 'border-box' }} />
              </div>
              {errorMod && <p style={{ color: '#dc2626', fontSize: '0.85rem', margin: 0 }}>{errorMod}</p>}
              <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
                <button onClick={() => setMostrarModal(false)} disabled={guardando} style={{ padding: '0.7rem 1.2rem', borderRadius: '8px', border: '1px solid #d1d5db', background: 'white', color: '#374151', fontWeight: '600', fontSize: '0.9rem', cursor: 'pointer' }}>Cancelar</button>
                <button onClick={handleGuardar} disabled={guardando} style={{ padding: '0.7rem 1.2rem', borderRadius: '8px', border: 'none', background: '#2563eb', color: 'white', fontWeight: '600', fontSize: '0.9rem', cursor: guardando ? 'not-allowed' : 'pointer', opacity: guardando ? 0.7 : 1 }}>
